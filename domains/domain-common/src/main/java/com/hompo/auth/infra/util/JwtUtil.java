...
public class JwtUtil {
    ...
    public static boolean verifyToken(String token, JwtSecretDto jwtSecretDto) throws JWTVerificationException {
        ...
        return Objects.equals(verifier.verify(token).getSignature(), jwtSecretDto.secretKey());
    }

    /**
     * 토큰의 signature를 확인하고, 아직 만료되지 않았을 때만 true압니다.
     * */
    public static boolean verifyTokenNotExpired(String token, JwtSecretDto jwtSecretDto) throws JWTVerificationException {
        JWTVerifier verifier = JWT.require(Algorithm.HMAC512(jwtSecretDto.secretKey())).build();
